...
class Dataset:
    def __init__(
        ...
        *,
        ...
        use_deepseek_encode=False,
    ):
        origin_samples = []
        if use_deepseek_encode:
            # --- DeepSeek encode逻辑 ---
            import sys
            sys.path.append("/gfs/space/chatrl/users/djy/0121/DeepSeek-V3.2/encoding")
            ...
            for data in read_file(path):
                ...
                # 修复结构（可选，按你convert_to_slime.py的fix_data_structure逻辑）
                fixed_tools = []
                if tools:
                    for t in tools:
            ...
            for data in read_file(path):
                ...
                # Fix compatibility: Parse tool arguments if they are strings (OpenAI format)
                if isinstance(prompt, list):
                    for message in prompt:
                        if isinstance(message, dict) and "tool_calls" in message and isinstance(message["tool_calls"], list):
                            for tc in message["tool_calls"]:
                                if isinstance(tc, dict):
                                    target = tc.get("function", tc)
                                    if isinstance(target, dict) and "arguments" in target and isinstance(target["arguments"], str):
                                        try:
                                            parsed = json.loads(target["arguments"])
        ...
        print(f"✅ Loaded {len(origin_samples)} samples from {path}.")
        print(f"   > Sample 0: {origin_samples[0]}")
